...
for c in C_list:
    score_list = []
    for g in gamma_list:
        clf = svm.SVC( C = c, gamma= g, kernel='rbf', decision_function_shape = 'ovr').fit(X_train,y_train)
        clf_list.append(clf)
...
pickle.dump((X_train_org, scaler, clf_max), open('depth_rater_multi.pkl', 'wb'))
